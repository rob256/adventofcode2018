...
def get_answer(input_text: str) -> Tuple[int, int]:
    ...
    occupied_squares = {(arrow.x, arrow.y): arrow for arrow in arrows}
    print(arrows)
    while True:
        print('Next Move')
        for arrow in sort_arrows_by_coord(arrows):
            ...
            if (new_x, new_y) in occupied_squares:
                crashed_arrow = occupied_squares[new_x, new_y]
                crashed_arrow.crashed = True
                arrow.crashed = True
                arrows.remove(crashed_arrow)
                arrows.remove(arrow)
                del occupied_squares[(x, y)]
                del occupied_squares[(new_x, new_y)]
            else:
                del occupied_squares[(x, y)]
                occupied_squares[new_x, new_y] = arrow
            print(arrows)
        if len(arrows) == 1:
            return arrows[0].x, arrows[0].y
