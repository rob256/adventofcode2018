...
def parse_board_input(input_text: str, elf_attack_power: int) -> Tuple[Board, List[Elf], List[Goblin]]:
    ...
    for y, line in enumerate(input_text.splitlines()):
        for x, text in enumerate(line):
            if text == 'G':
                ...
                board_piece = Elf(x, y)
                board_piece.attack_power = elf_attack_power
...
def get_answer(input_text: str) -> int:
    ...
    while True:
        try:
            board, elves, goblins = parse_board_input(input_text, elf_attack_power)
            print_board(board)
            game_move = 0
